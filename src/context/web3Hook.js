...
export const useSigningWeb3Client = () => {
  ...
  const updateBalances = async () => {
    ...
    try {
      ...
      const cacoContract = new web3.eth.Contract(ABI.CACO, config.CACO_CONTRACT);
      let cacoBalance = await cacoContract.methods.balanceOf(accounts[0]).call();
      cacoBalance = web3.utils.fromWei(cacoBalance);
  ...
  const getTotalPresaleAmount = async () => {
    ...
    try {
      const cacoContract = new web3.eth.Contract(ABI.CACO, config.CACO_CONTRACT);
      let presaleAmount = await cacoContract.methods.balanceOf(config.PRESALE_FACTORY).call();
  ...
  const setFeesOnNormalTransfer = async (enabled) => {
    ...
    try {
      const accounts = await web3.eth.getAccounts();
      if (accounts.length === 0) return { success: false }
      const MagicContract = new web3.eth.Contract(ABI.CACO, config.CACO_CONTRACT);
  ...
  const getCACOPriceInWeb3 = async () => {
    if (!web3) return { success: false }
    try {
      // const path = [];
      // path.push(config.CACO_CONTRACT);
      // path.push(config.WBNB_CONTRACT);
      // path.push(config.BUSD_CONTRACT);
      ...
      return {
        success: true,
        CACOPrice: 0.07 // web3.utils.fromWei(amountOut[amountOut.length - 1], to_decimal)
      }
    } catch (error) {
      console.log('[BUSD Error] = ', error);
      return {
